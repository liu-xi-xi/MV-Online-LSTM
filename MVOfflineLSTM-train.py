...
from keras.layers import Dense,concatenate,LSTM,Activation
...
input_1=tf.keras.layers.Input(shape=(1,6))#------->点位1子网络的输入
input_2=tf.keras.layers.Input(shape=(1,6))#------->点位2子网络的输入
input_3=tf.keras.layers.Input(shape=(1,6))#------->点位3子网络的输入
input_4=tf.keras.layers.Input(shape=(1,6))#------->点位4子网络的输入
input_5=tf.keras.layers.Input(shape=(1,6))#------->点位5子网络的输入

hidden_1 = LSTM(25,return_sequences=True)(input_1)#每一个子网络
hidden_2 = LSTM(25,return_sequences=True)(input_2)
hidden_3 = LSTM(24,return_sequences=True)(input_3)
hidden_4 = LSTM(26,return_sequences=True)(input_4)
hidden_5 = LSTM(26,return_sequences=True)(input_5)

concat = concatenate([hidden_1,hidden_2,hidden_3,hidden_4,hidden_5])
out = Dense(30)(concat)
model=Model(inputs=[input_1,input_2,input_3,input_4,input_5],outputs=[out])
model.compile(loss='mae',optimizer='adam')
history = model.fit(x=[trainX1,trainX2,trainX3,trainX4,trainX5],y=trainY,epochs=300,batch_size=5, shuffle=False)
model.save('MV-LSTM.h5')
...
print("耗氧量平均平方误差为：" + str(r2_score(c6_y_4,l6_y_4)))
