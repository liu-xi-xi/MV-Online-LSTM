...
from keras.layers import Input, LSTM, Dense, Activation, Dot, Concatenate
...
def self_attention(input_tensor):
    attention_weights = Dot(axes=[2, 2])([input_tensor, input_tensor])
    attention_weights = Activation('softmax')(attention_weights)
    context_vector = Dot(axes=[2, 1])([attention_weights, input_tensor])
...
def create_model(input_shape, output_shape):
    ...
    # LSTM 层
    lstm_output = LSTM(units=37, return_sequences=True)(inputs)

    # 自注意力机制
    context_vector = self_attention(lstm_output)

    # 拼接注意力输出和 LSTM 输出
    attention_output = Concatenate(axis=-1)([lstm_output, context_vector])

    # 全连接层
    outputs = Dense(output_shape, activation='sigmoid')(attention_output)
...
# 创建模型实例
input_shape = (trainX.shape[1], trainX.shape[2]) # 输入数据形状
output_shape = trainY.shape[2]    # 输出数据形状
model = create_model(input_shape, output_shape)

# 编译模型
model.compile(optimizer='adam', loss='mae')
model.fit(trainX, trainY, epochs=300, batch_size=5, shuffle=False)
model.save('AT-LSTM3.h5')

yhat = model.predict(testX)
train = data[:286,:]
yhat_squeezed = np.squeeze(yhat, axis=1)
# 拼接 train 和 yhat 数组
arr3 = np.concatenate((train, yhat_squeezed), axis=0)
